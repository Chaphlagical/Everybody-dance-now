...
target_img = cv2.imread('./data/target/'+target+'/train/train_label/00001.png')[:,:,0]
target_img_rgb = cv2.imread('./data/target/'+target+'/train/train_img/00001.png')
source_img = cv2.imread('./data/target/'+target+'/train/train_label/00001.png')[:,:,0]
source_img_rgb = cv2.imread('./data/target/'+target+'/train/train_img/00001.png')

path = './data/source/'+source+'/test_label_ori/'
save_dir = Path('./data/source/'+source)
output = save_dir.joinpath('test_label')
output.mkdir(exist_ok=True)
head_dir = save_dir.joinpath('test_head')
head_dir.mkdir(exist_ok=True)
pose_dir = Path('./data/source/'+source+'/pose_source.npy')
